...
class LineChart extends AbstractChart {
  ...
  renderDots = (config) => {
    const {
      data, width, height, paddingTop, paddingRight, onDataPointClick
    } = config;
    const output = [];
  ...
  renderBezierShadow = (config) => {
    ...
    data.map((dataset, index) => {
      const d = `${this.getBezierLinePoints(dataset, config)} L${paddingRight
        + ((width - paddingRight) / dataset.data.length) * (dataset.data.length - 1)},${(height / 4)
        * 3
        + paddingTop} L${paddingRight},${(height / 4) * 3 + paddingTop} Z`;
      output.push(<Path key={index} d={d} fill="url(#fillShadowGradient)" strokeWidth={0} />);
    });
    return output;
  };
